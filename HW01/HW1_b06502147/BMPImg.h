...
public:
	...
	bool rotate() {
		...
		unsigned int p;
		unsigned char* copy;
		copy = new unsigned char[getPxlNum() * getBytesPerPixel()];
		for(int i = 0;i < getWidth();i++){
			for(int j = getHeight();j > 0;j--){
				for(int k = 0;k < 3;k++)copy[3*(getHeight()-j)+3*getHeight()*i+k]=data[3*i+3*getWidth()*j+k];
			}
		}
		delete []data;
		data = copy;
		copy = NULL;
		swap(*((unsigned int*) (header.pFlag(5))),*((unsigned int*) (header.pFlag(6))));
